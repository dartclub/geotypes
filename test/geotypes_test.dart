  ...
  test(
    ...
    () {
      ...
      for (var geom in geomColl.geometries) {
        ...
        Iterable expandRecursively(List inner) {
          if (inner is List<Position>) {
            return inner;
          } else {
            return inner
                .expand((el) => el is List ? expandRecursively(el) : el);
          }
        }

        var expanded = expandRecursively(geom.coordinates);
        expect(
          expanded.first,
          Position(100, 0),
        );
